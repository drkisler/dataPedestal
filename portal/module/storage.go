...
func (dbs *TStorage) GetPluginByHostID(strHostID string) ([]TPlugin, error) {
	strSQL := "select plugin_uuid, plugin_config, run_type " +
		"from plugins where host_uuid=?"
	rows, err := dbs.Queryx(strSQL, strHostID)
	if err != nil {
		return nil, err
	}
	defer func() {
	...
	for rows.Next() {
		var p TPlugin
		if err = rows.Scan(&p.PluginUUID, &p.PluginConfig, &p.RunType); err != nil {
			return nil, err
		}
...
func (dbs *TStorage) GetPluginNames(p *TPlugin /*, pageSize int32, pageIndex int32*/) (plugins []TPluginInfo, total int, err error) {
	...
	if p.PluginType == "全部插件" {
		strSQL = "select plugin_name,plugin_uuid " +
			"from (select plugin_uuid, plugin_name from plugins where user_id= ? order by plugin_type,plugin_name) t "
		if rows, err = dbs.Queryx(strSQL, p.UserID); err != nil {
			return nil, -1, err
		}
	} else {
		strSQL = "select plugin_name,plugin_uuid " +
