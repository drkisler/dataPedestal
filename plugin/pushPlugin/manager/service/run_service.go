...
func CreateMyPushPlugin() common.IPlugin {
...
func (mp *TMyPlugin) Load(config string) common.TResponse {
	...
	if pullServCfg.DBConnection == "" {
		return *common.Failure("数据库连接信息不能为空，请确认配置是否正确")
	}
	// pullServCfg.
	if pullServCfg.HostPubUrl == "" {
		return *common.Failure("未能获取到订阅地址，请确认配置是否正确")
	}

	strConn, err := common.DecryptAES(pullServCfg.DBConnection, common.GetDefaultKey())
	if err != nil {
		return *common.Failure(fmt.Sprintf("解密数据库连接信息失败:%s", err.Error()))
	}
